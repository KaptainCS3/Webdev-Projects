...
const Main: React.FC<MainProps> = ({ result, font }) => {
  return (
    <div>
      {result.map((el, index) => {
          ...
          return (
            <div key={index}>
              <div className="flex justify-between items-center py-3">
                ...
                <span className="border-t-[1px] w-[75%] dark:border-t-white block mt-[0.25rem]"></span>
              </div>
              ...
              {e.definitions.map((list, listIndex) => {
                return (
                  <ul key={listIndex} className="list-none">
                    ...
                    {list.example ? (
                      <small className="text-[#A0A0A0] text-sm pl-6 -mt-2 block">
                        "{list.example}"
                      </small>
                    ) : null}
                  </ul>
              ...
              <div className="px-2 sm:flex flex-wrap justify-evenly items-center py-3 overflow-x-auto">
                {e.synonyms.map((synonym, synonymIndex) => {
                  return (
                    <React.Fragment key={synonymIndex}>
                      {synonym ? (
                        <span className="text-[#A641F5] font-medium px-2 cursor-pointer break-words">
                          {synonym}
                        </span>
                      ) : (
                        ""
